...
- 同義語グループの aliases もハイライト対象に含まれる
- alias クリック時は `aliasToTermMap`（alias → 代表用語の逆引きマップ）で代表用語に解決
- 代表用語選択中は、同グループの aliases も選択色（黄色）で表示
...
- 用語のハイライト（青色背景）
- 選択中の用語のハイライト（黄色背景）
- 同義語（aliases）のハイライト（代表用語と同色）
- 用語/alias クリックで `onTermClick` コールバック呼び出し（alias は代表用語名に解決）
