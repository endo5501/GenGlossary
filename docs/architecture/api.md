...
### excluded_terms.py (除外用語API - v5)
```python
router = APIRouter(prefix="/api/projects/{project_id}/excluded-terms", tags=["excluded-terms"])
...
async def list_excluded_terms(
    ...
    return ExcludedTermListResponse(
        items=ExcludedTermResponse.from_models(terms),
        total=len(terms),
    )

@router.post("", response_model=ExcludedTermResponse)
async def create_excluded_term(
    project_id: int = Path(...),
    request: ExcludedTermCreateRequest = Body(...),
    project_db: sqlite3.Connection = Depends(get_project_db),
) -> JSONResponse:
    """除外用語を追加（既存の場合は200、新規の場合は201）"""
    # ON CONFLICT DO NOTHINGで重複を許容
    # 新規追加時は201、既存時は200を返す
    ...

@router.delete("/{term_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_excluded_term(
    project_id: int = Path(...),
    term_id: int = Path(...),
    project_db: sqlite3.Connection = Depends(get_project_db),
) -> None:
    """除外用語を削除"""
    success = delete_excluded_term(project_db, term_id)
    if not success:
        raise HTTPException(status_code=404, detail="Excluded term not found")
```

### _synonym_helpers.py (同義語ヘルパー)

Provisional/Refined APIエンドポイントで共通使用するヘルパーモジュール。

```python
from genglossary.db.synonym_repository import list_groups

def build_aliases_map(conn: sqlite3.Connection) -> dict[str, list[str]]:
    """primary_term_text → 非代表メンバーテキストのリスト を構築"""
    groups = list_groups(conn)
    aliases_map: dict[str, list[str]] = {}
    for group in groups:
        aliases = [m.term_text for m in group.members if m.term_text != group.primary_term_text]
        if aliases:
...
async def export_markdown(
    ...
    同義語グループが設定されている場合、代表用語に **別名** セクションを出力。
    """
...
def _validate_file_name(file_name: str) -> str:
    ...
    - 制御文字を拒否:
      ...
      - C1制御文字: U+007F-U+009F
    - 双方向上書き/ゼロ幅文字を拒否（ファイル名偽装防止）:
      - ゼロ幅文字: U+200B-U+200F, U+FEFF
      - Bidi制御: U+202A-U+202E, U+2066-U+2069
      - 書式制御: U+061C, U+2060
    ...
    - 末尾スペース/ドット/Unicode空白を拒否（各セグメント、Windows互換性）:
      - ASCII空白/ドット: ' ', '.'
      - Unicode空白: U+00A0, U+2000-U+200A, U+3000
    - Windows予約デバイス名を拒否（ベースネームのみ）:
...
**Excluded Terms API (除外用語管理) - 3エンドポイント (v5):**
- `GET /api/projects/{project_id}/excluded-terms` - 除外用語一覧取得
