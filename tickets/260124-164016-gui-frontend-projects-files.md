...
- [x] **Red**: 先にUIテスト追加（`frontend/src/__tests__/projects-page.test.tsx`）— プロジェクト一覧の表示/選択、作成モーダルのバリデーション、API呼び出しモック、空状態表示、ファイルタブ表示をRTLで失敗させる
- [x] テスト失敗を確認（Red完了）
- [x] Home screen: project list with stats (last updated, docs count, issues count) and selection to show right-side summary card（`docs/architecture.md`のUIフローに反映）
- [x] Implement project create/duplicate/delete dialogs calling API; refresh list and toast results
- [x] Project detail shell with left nav highlighting and breadcrumbs from global layout
- [x] Files tab: list documents with status + updated_at; actions to add/replace files and trigger diff scan
